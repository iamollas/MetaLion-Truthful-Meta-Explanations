...
def generate_per_instance_importances(models, X, y, framework='kernel_shap'):
    """
    ...
    for model in models:
        def predict_function(x):
                ...
                return [inst[0] for inst in prediction]
        def my_predict(x):
            return np.array([i for i in predict_function(x)])
        if framework == 'kernel_shap':
            explainer = shap.KernelExplainer(my_predict, X)
            ...
            for index, (skip, row) in enumerate(X.iterrows()):
                ...
                # If is multiclass, choose explanation for the correct class
                exp = explainer.explain_instance(row, my_predict, num_features=len(X.columns),
...
class GlobalFeatureMetric:
    ...
    def gradual_perturbation(self, model, X, y, importances_orig, column_transformer, preprocessing_pipeline=None,
                             resolution=10, count_per_step=5, plot=True, task='c', reshape=None):
        """
        ...
        """
        if len(reshape) == 2:
            dataset = np.array(X.values).reshape((len(X.values),reshape[0],reshape[1]))
        elif len(reshape) == 3:
            dataset = np.array(X.values).reshape((len(X.values),reshape[0],reshape[1],reshape[2]))
        else:
            dataset = X    
        if task == 'c':
            baseline_predictions = [1 if inst[0]>=0.5 else 0 for inst in model.predict(dataset)]
            baseline_accuracy = accuracy_score(y, baseline_predictions)
        else:
            baseline_predictions = [inst[0] for inst in model.predict(dataset)]
            baseline_accuracy = r2_score(y, baseline_predictions)
        ...
        for importances in intermediate_importances:
            this_step_accuracies = self.gradual_perturbation_step(model=model, X=X, y=y,
                                                                ...
                                                                baseline_accuracy=baseline_accuracy, task=task, 
                                                                reshape=reshape)
    ...
    def gradual_perturbation_step(self, model, X, y, baseline_accuracy, importances, column_transformer,
                                  ...
                                  count_per_step=5, task='c', reshape=None):
        ...
        for i in range(count_per_step):
            ...
            if len(reshape) == 2:
                dataset = np.array(dataset.values).reshape((len(dataset.values),reshape[0],reshape[1]))
            elif len(reshape) == 3:
                dataset = np.array(dataset.values).reshape((len(dataset.values),reshape[0],reshape[1], reshape[2]))
            if task == 'c':
                predictions = [1 if inst[0]>=0.5 else 0 for inst in model.predict(dataset)]
                this_step_accuracies.append(accuracy_score(y, predictions))
            else:
                predictions = [inst[0] for inst in model.predict(dataset)]
                this_step_accuracies.append(r2_score(y, predictions))
